...
class Beam_Spring:
    ...
    def model_ode(cls, t, X, T, F):
        ...
        _X = np.concatenate(
            (x[None, :, np.newaxis], xdot[None, :, np.newaxis]), axis=-1)
        _force = force[None, :, None]
        xddot = cls.pred_acc(_X, _force)

        Xdot = np.concatenate((xdot, xddot[0]))
        return Xdot
    ...
    def model_sens_ode(cls, t, ic, T, F):
        ...
        # Force derivatives
        def forcing(t, T, F):
            return jnp.array([F * jnp.sin(2 * jnp.pi / T * t), 0])
        _dforce_dT = jax.jacrev(forcing, argnums=1)(t, T, F)
        _dforce_dF = jax.jacrev(forcing, argnums=2)(t, T, F)
        ...
        _xddotdx = jax.jacrev(cls.pred_acc, argnums=0)(
            _X, _force)[:, :, 0, :, 0]
        _xddotdxdot = jax.jacrev(cls.pred_acc, argnums=0)(
            _X, _force)[:, :, 0, :, 1]

        _dgdX = np.zeros((twoN, twoN))
        _dgdX[:N, N:] = np.eye(N)  # dx/dt = xdot
        _dgdX[N:, :N] = _xddotdx  # d(xddot)/dx
        _dgdX[N:, N:] = _xddotdxdot  # d(xddot)/d(xdot)

        # Partial derivatives with respect to parameters
        _dgdT = np.concatenate([np.zeros(N), _dforce_dT])
        _dgdF = np.concatenate([np.zeros(N), _dforce_dF])

        # Sensitivity calculations
        dXdX0dot = _dgdX @ dXdX0.reshape(twoN, twoN)
        dXdTdot = _dgdX @ dXdT.reshape(twoN, 1) + _dgdT.reshape(-1, 1)
        dXdFdot = _dgdX @ dXdF.reshape(twoN, 1) + _dgdF.reshape(-1, 1)
    ...
    def time_solve(cls, omega, F, T, X, pose_base, cont_params, sensitivity=True, fulltime=False):
        def model_sens_ode(t, ic, args):
            ...
            _dgdX = jnp.zeros((twoN, twoN))
            _dgdX = _dgdX.at[:N, N:].set(jnp.eye(N))
            _dgdX = _dgdX.at[N:, :N].set(_xddotdx[0])
            _dgdX = _dgdX.at[N:, N:].set(_xddotdxdot[0])

            # Partial derivatives with respect to parameters
            _dgdT = jnp.concatenate([jnp.zeros(N), _dforce_dT])
            _dgdF = jnp.concatenate([jnp.zeros(N), _dforce_dF])
            ...
            return jnp.concatenate([Xdot, dXdX0dot.flatten(), dXdTdot.flatten(), dXdFdot.flatten()])
        ...
        term = ODETerm(model_sens_ode)
        solver = Dopri5()
        saveat = SaveAt(ts=t)
        _sol = diffeqsolve(
            term, solver, t0=t[0], t1=t[-1], dt0=t[1]-t[0], y0=all_ic, args=(T, F), saveat=saveat, stepsize_controller=PIDController(rtol=rel_tol, atol=rel_tol), max_steps=1000000
        )
        sol = _sol.ys
    ...
    @classmethod
    def model_ode_first_pnt(cls, t, X, T, F):
        # State equation of the mass spring system. Xdot(t) = g(X(t))
        x = X[: cls.ndof_free]
        xdot = X[cls.ndof_free:]
        KX = cls.K @ x
    ...
    @classmethod
    def model_sens_ode_first_pnt(cls, t, ic, T, F):
        """
        Augmented ODE of model + sensitivities, to be solved together
        System: Xdot(t) = g(X(t))
        Monodromy: dXdX0dot = dg(X)dX . dXdX0
    ...
    @classmethod
    def time_solve_first_pnt(cls, omega, F, T, X, pose_base, cont_params, sensitivity=True, fulltime=False):
        nsteps = cont_params["shooting"]["single"]["nsteps_per_period"]
        rel_tol = cont_params["shooting"]["rel_tol"]
        continuation_parameter = cont_params["continuation"]["continuation_parameter"]
        N = cls.ndof_free
        ...
        all_ic = np.concatenate(
            (X0, np.eye(twoN).flatten(), np.zeros(twoN), np.zeros(twoN)))
        sol = np.array(
            odeint(cls.model_sens_ode_first_pnt, all_ic, t, args=(
        ...
        # Jacobian construction depends on continuation parameter
        dHdX0 = M - np.eye(twoN)
        gX_T = cls.model_ode_first_pnt(T, Xsol[-1, :], T, F)
