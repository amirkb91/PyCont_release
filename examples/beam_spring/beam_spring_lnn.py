...
class Beam_Spring:
    ...
    @classmethod
    def model_sens_ode_orig(cls, t, ic, T, F):
        """
    ...
    @classmethod
    def time_solve_orig(cls, omega, F, T, X, pose_base, cont_params, sensitivity=True, fulltime=False):
        ...
        sol = np.array(
            odeint(cls.model_sens_ode_orig, all_ic, t, args=(
        ...
        print(
            f"Xsol: {Xsol.shape}, M: {M.shape}, dXdT: {dXdT.shape}, dXdF: {dXdF.shape}")
    ...
    def time_solve(cls, omega, F, T, X, pose_base, cont_params, sensitivity=True, fulltime=False):
        ...
        X0 = X + np.concatenate((pose_base, np.zeros(N)))
        t = np.linspace(0, T, nsteps + 1)
        Xsol = jode.odeint(cls.model_ode, X0, t,
                           T, F, rtol=rel_tol)

        def periodicity(X0, T, F):
            t = np.linspace(0, T, nsteps + 1)
            Xsol = jode.odeint(cls.model_ode, X0, t, T, F, rtol=rel_tol)
            H = Xsol[-1, :] - Xsol[0, :]
        ...
        # Monodromy
        dHdX = jax.jacrev(cls.model_ode, argnums=0)(
            Xsol[0, :], T, T, F)
        dHdX0 = dHdX @ np.eye(twoN).reshape(twoN, twoN) - np.eye(twoN)

        # Jacobian construction depends on continuation parameter
        # dHdX0 = M - np.eye(twoN)
        gX_T = cls.model_ode(Xsol[-1, :], T, T, F)

        dXdT = jax.jacrev(cls.model_ode, argnums=2)(Xsol[-1, :], T, T, F)
        dXdF = jax.jacrev(cls.model_ode, argnums=3)(Xsol[-1, :], T, T, F)
        if continuation_parameter == "frequency":
            # For frequency continuation, include period sensitivity (dH/dT)
            dHdT = gX_T + dXdT
            J = np.concatenate((dHdX0, dHdT.reshape(-1, 1)), axis=1)
