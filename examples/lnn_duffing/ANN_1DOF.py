...
class Conservative_MLP(hk.Module):
    """
    Define the 1DOF conservative lagrangian neural network with this class.
    ...
    def __init__(self, MODEL, name=None):
        ...
        # Output layer without activation function to enable the network to freely predict negative Lagrangian values
        ...
        self.mlp = hk.Sequential(lag_layers)

    # Function to apply the network to input data
    def __call__(self, x):
        return self.mlp(x)
...
class Conservative_LNN(BaseModel):
    ...
    def _eom(self):
        ...
        def eom(L, x, f):
            ...
            M = jax.hessian(L, 1)(q, q_t)
            KQ = jax.jacobian(L, 0)(q, q_t)
            ...
            return jnp.concatenate([q_t, q_tt.squeeze(-1)])
    ...
    def _dynamics(self):
        ...
        def dynamics(params):
            ...
            def zeros(x):
                ...
                z = jnp.zeros_like(x)
                # NN output for 0 displ. & vel. && Jacobian of params wrt z
                z1 = net.apply(params, z)
                dz1 = jax.jacrev(net.apply, 1)(params, z)
                # dzz1 = jax.jacfwd(jax.jacrev(net.apply, 1), 0)(params, z)
            ...
            def lagrangian(qq, qq_t):
                ...
                return jnp.squeeze(net.apply(params, state), axis=-1)
    ...
    def _loss(self):
        ...
        def loss_fn(params, batch):
            ...
            pred = jax.vmap(partial(eom, lagrangian))(q, f)
            z1, dz1 = jax.vmap(partial(zeros))(q)

            # Enforce true predictions & respect BCs
